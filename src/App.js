...
function App() {
  ...
  useEffect(() => {
    ...
    .then(data => setQuizData(trimValues(data.results)))
  }, [startNewGame])
      ...
      if (prevData[i].question === question) {
        for (let j = 0; j < prevData[i].answers.length; j++) {
          if (prevData[i].answers[j].value === answer) {
            prevData[i].answers[j].isSelected = !prevData[i].answers[j].isSelected;
          } else {
    ...
    setQuizData((prevData) => {
      ...
      for (let i = 0; i < prevData.length; i++) {
        for (let j = 0; j < prevData[i].answers.length; j++) {
          ...
          if (prevData[i].answers[j].isSelected && prevData[i].answers[j].value === prevData[i].correct_answer) {
